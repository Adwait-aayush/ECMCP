...
func (s *ProductService) convertToProductResponse(product *models.Product) (*dto.ProductResponse, error) {
...
func (s *ProductService) GetProducts(page, limit int) ([]dto.ProductResponse, *utils.PaginationMeta, error) {
	...
	if err := s.db.Preload("Category").Preload("Images").Where("is_active = ? ", true).Limit(limit).Offset(offset).Find(&products).Error; err != nil {
		return nil, nil, err
	}
	responses := make([]dto.ProductResponse, len(products))
	for i := range products {
		resp, err := s.convertToProductResponse(&products[i])
		if err != nil {
			return nil, nil, err
		}
		responses[i] = *resp
...
func (s *ProductService) GetProduct(id uint) (*dto.ProductResponse, error) {
	...
	return s.convertToProductResponse(&product)
}
